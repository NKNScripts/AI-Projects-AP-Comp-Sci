...
import aima.core.environment.vacuum.VacuumEnvironmentC;
...
public class VacuumController extends AgentAppController {
	...
	protected AbstractEnvironment env = null;
	protected AbstractAgent agent = null;
	protected boolean isPrepared = false;
        protected boolean reflexCount = false;
	...
	public boolean isPrepared() {
                if(reflexCount) return true;
		return isPrepared && env != null && !env.isDone();
	}
		...
		try {
                        if(reflexCount){
                            ...
                            for(int i = 0; i < 2; i++){
                                for(int j = 0; j < 2; j++){
                                    for(int k = 0; k < 2; k++){
                                        env = new VacuumEnvironmentC(i,j);
                                        ((VacuumEnvironmentC)env).setAgentLocation(k == 0 ? true : false, agent);
                                        if (env != null && agent != null) {
                                            frame.getEnvView().setEnvironment(env);
                                            env.addAgent(agent);
                                            isPrepared = true;
                                        ...
                                        logger.log("<Vaccum Scenario: " + i + "," + j + ">");
                                        while (!env.isDone() && !frame.simulationPaused()) {
                                            Thread.sleep(500);
                                            env.step();
                                        }
                                        totalPerformance += env.getPerformanceMeasure(agent);
                            ...
                            logger.log("Average Performance: " + totalPerformance / 8);
                        }
                        else
                            while (!env.isDone() && !frame.simulationPaused()) {
                                    Thread.sleep(500);
